...
public class AttributeOverlayHandler {
    ...
    private static void actuallyRenderAttributeOverlay(PoseStack poseStack, int screenWidth, int screenHeight, LivingEntity entity, Font font, ItemRenderer itemRenderer) {
        ...
        for (int i = 0; i < tooltipComponents.size(); i++) {
            TooltipRenderHelper.renderTooltip(poseStack, posX, posY + 29 * i, TextComponent.EMPTY, tooltipComponents.get(i));
        }
    }
    ...
    private static List<HorseAttributeTooltip> buildTooltipComponents(LivingEntity entity) {
        ...
        if (entity.getAttributes().hasAttribute(Attributes.MAX_HEALTH)) {
            tooltipComponents.add(HorseAttributeTooltip.healthTooltip(entity.getAttributeValue(Attributes.MAX_HEALTH), entity instanceof AbstractHorse));
        }
        if (!(entity instanceof Llama) || HorseExpert.CONFIG.get(ClientConfig.class).allLlamaAttributes) {
            if (entity.getAttributes().hasAttribute(Attributes.MOVEMENT_SPEED)) {
                tooltipComponents.add(HorseAttributeTooltip.speedTooltip(entity.getAttributeValue(Attributes.MOVEMENT_SPEED), entity instanceof AbstractHorse));
            }
            if (entity.getAttributes().hasAttribute(Attributes.JUMP_STRENGTH)) {
                tooltipComponents.add(HorseAttributeTooltip.jumpHeightTooltip(entity.getAttributeValue(Attributes.JUMP_STRENGTH), entity instanceof AbstractHorse));
            }
        }
